...
from tkinter import ttk  #Provides access to the Tk themed widget
import sqlite3
...
class Contacts:
	...
	def create_label_frame(self):
		labelframe = LabelFrame(self.root, text='Create New Contact', bg='sky blue', font='helvetica 12')
		...
		Button(labelframe, text='Add Contact', command=self.on_add_contact_button_clicked, bg='blue',fg='white').grid(row=4,column=2,sticky=E,pady=5,padx=5)
	...
	def on_delete_selected_button_clicked(self):
		...
		try:
			...
			self.message['text'] = "No item is selected to delete"
			return
	...
	def on_modify_selected_button_clicked(self):
		...
		try:
			...
			self.message['text'] = "No item is selected to modify"
			return
	...
	def open_modify_window(self):
		...
		Label(self.transient, text="Name:").grid(row=0, column=2)
		Entry(self.transient, textvariable=StringVar(
			self.transient, value=name), state='readonly').grid(row=0, columns=3)

		Label(self.transient, text="Old Contact Number:").grid(row=1, column=2)
		Entry(self.transient, textvariable=StringVar(
			self.transient, value=old_number), state='readonly').grid(row=1, columns=3)

		Label(self.transient, text="New Contact Number:").grid(row=2, column=2)
		new_phone_number_entry_widget = Entry(self.transient)
		new_phone_number_entry_widget.grid(row=2,column=3)
	...
	def update_contacts(self, newphone, old_phone, name):
		...
		self.message['text'] = 'Phone Numeber of {} modified'.format(name)
		self.view_contacts()
